...
fn main() {
    ...
    for line in reader.lines() {
        ...
        if is_safe(&levels) && (is_asc(&levels) || is_desc(&levels)) {
            safe_count += 1;
            //println!("{:?} Safe", levels);
        } else if is_safe_by_dropping_single_level(&levels) {
            safe_count += 1;
            //println!("{:?} Safe", levels);
        }
    }
...
fn is_safe_by_dropping_single_level(levels: &Vec<i32>) -> bool {
    for i in 0..levels.len() {
        let mut new_levels = levels.clone();
        new_levels.remove(i);
        if is_safe(&new_levels) && (is_asc(&new_levels) || is_desc(&new_levels)) {
            // println!("{:?} Safe by dropping {}", levels, levels[i]);
            return true;
        }
    }
    false
